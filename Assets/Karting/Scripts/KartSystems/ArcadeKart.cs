...
namespace KartGame.KartSystems
{
    public class ArcadeKart : MonoBehaviour
    {
        ...
        public float AirborneReorientationCoefficient = 20.0f;

        [Header("Drifting")]
        [Range(0.00f, 1.0f), Tooltip("The grip value when drifting.")]
        public float DriftGrip = 1.0f;
        [Range(0.0f, 10.0f), Tooltip("Additional steer when the kart is drifting.")]
        public float DriftAdditionalSteer = 0.0f;
        [Range(0.0f, 30.0f), Tooltip("The higher the angle, the easier it is to regain full grip.")]
        public float MinAngleToFinishDrift = 30.0f;
        [Range(0.00f, 0.99f), Tooltip("Mininum speed percentage to switch back to full grip.")]
        public float MinSpeedPercentToFinishDrift = 0.0f;
        [Range(0.0f, 20.0f), Tooltip("The higher the value, the easier it is to control the drift steering.")]
        public float DriftControl = 0.0f;
        ...
        public ParticleSystem DriftSparkVFX = null;
        ...
        [Tooltip("VFX that will be placed on the wheels when drifting.")]
        public GameObject DriftTrailPrefab = null;
        ...
        public GameObject NozzleVFX = null;
        ...
        public float SuspensionSpring = 10000.0f;
        ...
        float longDist = 30f;
        Vector3 target;
        public float hitConeNumber = 0;
        public float hitWallNumber = 0;
        ...
        public void goWhere(string goName)
        {
            // 找到目标物体的位置
            Vector3 pos = GameObject.Find(goName).transform.position;

            // 使用 Rigidbody 来移动车辆而不是直接修改 transform.position
            ...
            // 确保车辆可以正确停留在目标位置
            //SetCanMove(false); // 如果有控制车辆移动的布尔变量，可以在这里禁用移动
        }
