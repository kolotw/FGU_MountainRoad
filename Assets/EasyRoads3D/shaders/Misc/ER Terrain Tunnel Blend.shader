	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float2 uv4_DetailAlbedoMap = IN.ase_texcoord8.zw * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
				float4 lerpResult41 = lerp( ( tex2D( _MainTex, uv_MainTex ) * _Color ) , ( tex2D( _DetailAlbedoMap, uv4_DetailAlbedoMap ) * _Color ) , IN.ase_color.r);
				...
				float3 unpack36 = UnpackNormalScale( tex2D( _BumpMap, uv4_DetailAlbedoMap ), _BumpScale );
				unpack36.z = lerp( 1, unpack36.z, saturate(_BumpScale) );
				float3 tex2DNode36 = unpack36;
				float3 lerpResult42 = lerp( unpack5 , tex2DNode36 , IN.ase_color.r);
				
				float2 uv_Metallic = IN.ase_texcoord8.xy * _Metallic_ST.xy + _Metallic_ST.zw;
				float4 tex2DNode23 = tex2D( _Metallic, uv_Metallic );
				
				float3 Albedo = lerpResult41.rgb;
				float3 Normal = lerpResult42;
				float3 Emission = 0;
				float3 Specular = 0.5;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float2 uv4_DetailAlbedoMap = IN.ase_texcoord4.zw * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
				float4 lerpResult41 = lerp( ( tex2D( _MainTex, uv_MainTex ) * _Color ) , ( tex2D( _DetailAlbedoMap, uv4_DetailAlbedoMap ) * _Color ) , IN.ase_color.r);
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float2 uv4_DetailAlbedoMap = IN.ase_texcoord2.zw * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
				float4 lerpResult41 = lerp( ( tex2D( _MainTex, uv_MainTex ) * _Color ) , ( tex2D( _DetailAlbedoMap, uv4_DetailAlbedoMap ) * _Color ) , IN.ase_color.r);
