...
Shader "EasyRoads3D/ER Retaining Wall Blend"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			half _MainMetallicPower5;
			half _MainSmoothnessPower4;
			half _MainSmoothnessPower5;
						 ...
						 ) : SV_Target
			{
				...
				if (IN.ase_color.a < _Threshold) {
					temp_output_22_0 = (IN.ase_color.a / _Threshold);
				}
				...
				float3 unpack5 = UnpackNormalScale( tex2D( _BumpMap, texCoord39 ), _BumpScale1 );
				unpack5.z = lerp( 1, unpack5.z, saturate(_BumpScale1) );
				float2 texCoord37 = IN.ase_texcoord8.xy * float2( 1,1 ) + float2( 0,0 );
				float3 unpack23 = UnpackNormalScale( tex2D( _BumpMap2, texCoord37 ), _DetailNormalMapScale1 );
				unpack23.z = lerp( 1, unpack23.z, saturate(_DetailNormalMapScale1) );
				float3 lerpResult24 = lerp( unpack5 , unpack23 , temp_output_22_0);
				
				float2 uv_Metallic = IN.ase_texcoord8.xy * _Metallic_ST.xy + _Metallic_ST.zw;
				float4 tex2DNode2 = tex2D( _Metallic, uv_Metallic );
				float2 uv_MetallicGlossMap2 = IN.ase_texcoord8.xy * _MetallicGlossMap2_ST.xy + _MetallicGlossMap2_ST.zw;
				...
				float lerpResult27 = lerp( ( tex2DNode2.a * _MainSmoothnessPower4 ) , ( tex2DNode25.a * _MainSmoothnessPower5 ) , temp_output_22_0);
				
				float lerpResult29 = lerp( ( tex2DNode2.g * _OcclusionStrength4 ) , ( tex2DNode25.g * _OcclusionStrength5 ) , temp_output_22_0);
				
				float3 Albedo = lerpResult16.rgb;
				float3 Normal = lerpResult24;
				float3 Emission = 0;
				float3 Specular = 0.5;
