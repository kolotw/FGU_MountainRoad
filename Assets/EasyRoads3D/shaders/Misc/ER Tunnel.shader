// © 2021 EasyRoads3D
// This is a basic shader with "Cull Mode" set to "Off" so it renders double sided preventing light padding through into the tunnel as a result of holes in the terrain

Shader "EasyRoads3D/Misc/ER Tunnel"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float2 texCoord22 = IN.ase_texcoord8.zw * float2( 1,1 ) + float2( 0,0 );
				...
				float3 unpack38 = UnpackNormalScale( tex2D( _DetailNormalMap, texCoord37 ), _BumpScale1 );
				unpack38.z = lerp( 1, unpack38.z, saturate(_BumpScale1) );
				
				float2 uv_Metallic = IN.ase_texcoord8.xy * _Metallic_ST.xy + _Metallic_ST.zw;
				float4 tex2DNode23 = tex2D( _Metallic, uv_Metallic );
				
				float3 Albedo = ( ( ( tex2DNode1 * _Color ) * ( tex2D( _DetailAlbedoMap, texCoord22 ) * _Color1 ) ) * float4(2,2,2,2) ).rgb;
				float3 Normal = BlendNormal( unpack5 , unpack38 );
				float3 Emission = 0;
				float3 Specular = 0.5;
